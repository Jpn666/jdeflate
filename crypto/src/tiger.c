...
#define TIGER_ROUND(TT, A, B, C, X, M) \
	C ^= X; \
	A -= TT1(TT)[(C >> (0 << 3)) & 0xff] ^ \
		 TT2(TT)[(C >> (2 << 3)) & 0xff] ^ \
		 TT3(TT)[(C >> (4 << 3)) & 0xff] ^ TT4(TT)[(C >> (6 << 3)) & 0xff]; \
	B += TT4(TT)[(C >> (1 << 3)) & 0xff] ^ \
		 TT3(TT)[(C >> (3 << 3)) & 0xff] ^ \
		 TT2(TT)[(C >> (5 << 3)) & 0xff] ^ TT1(TT)[(C >> (7 << 3)) & 0xff]; \
	B *= M;


#define TIGER_PASS(TT, A, B, C, M)  \
	TIGER_ROUND(TT, A, B, C, x0, M) \
	TIGER_ROUND(TT, B, C, A, x1, M) \
	TIGER_ROUND(TT, C, A, B, x2, M) \
	TIGER_ROUND(TT, A, B, C, x3, M) \
	TIGER_ROUND(TT, B, C, A, x4, M) \
	TIGER_ROUND(TT, C, A, B, x5, M) \
	TIGER_ROUND(TT, A, B, C, x6, M) \
	TIGER_ROUND(TT, B, C, A, x7, M)


#if defined(TIGER_UNROLLED)

#define TIGER_COMPRESS(TT, block, state) \
	a = state[0]; \
	b = state[1]; \
	c = state[2]; \
	\
	x0 = block[0]; x1 = block[1]; x2 = block[2]; x3 = block[3]; \
	x4 = block[4]; x5 = block[5]; x6 = block[6]; x7 = block[7]; \
	\
	TIGER_PASS(TT, a, b, c, 5) TIGER_KEY_SCHEDULE \
	TIGER_PASS(TT, c, a, b, 7) TIGER_KEY_SCHEDULE \
	TIGER_PASS(TT, b, c, a, 9) \
	\
	for (z = 3; z < TIGER_PASSES; z++) { \
		TIGER_KEY_SCHEDULE \
		TIGER_PASS(TT, a, b, c, 9) \
		x = a; \
		a = c; \
		c = b; \
		b = x; \
...
#define TIGER_COMPRESS(TT, block, state) \
	...
	x4 = block[4]; x5 = block[5]; x6 = block[6]; x7 = block[7]; \
	\
	for (z = 0; z < TIGER_PASSES; z++) { \
		if (z) { \
			TIGER_KEY_SCHEDULE \
		} \
		TIGER_PASS(TT, a, b, c, ((!z) ? 5 : ((z == 1) ? 7 : 9))) \
		x = a; \
		a = c; \
		c = b; \
		b = x; \
...
void
tiger_getdigest(uint64 digest[3], const uint8* data, uintxx size)
{
	uint64 i;
	uint64 j;
	uint8 tmp[64];
...
static void
tiger_generate_table(uint64 table[1024], uint32 src[16], intxx passes)
{
	uint64 state[3];
	uint64 tmp[64];
	uint8 (*statech)[8];
